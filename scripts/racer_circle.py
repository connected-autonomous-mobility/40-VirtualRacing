...
def race(model_path, host, name):
    ...
    # load scene
    #msg = '{ "msg_type" : "load_scene", "scene_name" : "generated_track" }' # Virtual Racing League Track
    #msg = '{ "msg_type" : "load_scene", "scene_name" : "chicane_track" }' # RedPlex new track
    msg = '{ "msg_type" : "load_scene", "scene_name" : "generated_road" }' # Virtual Racing League Track
    ...
    maxcount = 10000000 #int(2.7/0.2) #2.7s 20Hz
    print("maxcount: ", maxcount)
    AI_START = True
    try:
        if AI_START:
            for i in range(maxcount): 
                client.send_controls(-0.7,0.7)
                time.sleep(0.2)
                print("AI driving straight", i)
            AI_START = False
